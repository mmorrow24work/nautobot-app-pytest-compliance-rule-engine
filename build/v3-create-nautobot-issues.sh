...
#
# Usage:
#   ./v3-create-nautobot-issues.sh OWNER/REPO
#   ./v3-create-nautobot-issues.sh mmorrow24work/nautobot-app-pytest-compliance-rule-engine
#
# Requires: gh CLI authenticated (gh auth login), jq, repo already created.
#
# Changes from v2:
#   - App/package renamed to match the new repo name:
#       app      nautobot-pytest-compliance -> nautobot-pytest-compliance-rule-engine
#       package  nautobot_pytest_compliance -> nautobot_pytest_compliance_rule_engine
#     The target repo itself is still the OWNER/REPO argument, not hardcoded.
#
# Changes from v1:
#   - Uses real GitHub Milestones (the /milestones API) instead of
#     "milestone:M0".."milestone:M8" LABELS. v1 only ever created labels;
#     the Milestones tab stayed empty.
...
#   - Fixes the M7 manual install issue, which in v1 got no milestone at all.
#
